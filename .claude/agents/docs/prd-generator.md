...
- **인증**: JWT (Access Token, 24시간 만료) + OAuth2 소셜 로그인(Google/Kakao)
- **DB**: PostgreSQL (스키마명 `TodoListDB`)
...
| **F011** | 소셜 로그인 | OAuth2(Google/Kakao) 로그인·자동가입·계정연동 | 로그인 페이지 |
...
- **JWT(24h)** · **OAuth2(Google/Kakao)**
- 검증: **Bean Validation**

### 🗄️ 데이터베이스
- **PostgreSQL** (스키마 `TodoListDB`)
